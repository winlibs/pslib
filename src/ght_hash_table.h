/*********************************************************************
 * Copyright (C) 2001, 2003-2004-2002,  Simon Kagstrom
 *
...
/*typedef unsigned  _ght_uint32_t;*/
#ifndef WIN32
 ...
 *        <TT>sizeof(ght_hash_entry_t) + sizeof(ght_hash_key_t) +
 *        key_size</TT>.
 *
 * @return a pointer to the allocated region, or NULL if the
 *         allocation failed.
 */
typedef void *(*ght_fn_alloc_t)(size_t size, void *data);
 ...
 */
typedef void (*ght_fn_free_t)(void *ptr, void *data);
...
typedef struct
{
  ...
  ght_fn_free_t fn_free;             /**< The function used for freeing entries */
	void *userdata;                    /**< User data passed to alloc and free */
