...
PUBLIC void *dlst_newnode(DLIST *l, int size)
/****************************************************************************
*
...
****************************************************************************/
{
	...
	if(l == NULL || l->malloc == NULL)
		return NULL;

	if ( NULL == (node = (PS_DLST_BUCKET*) l->malloc(NULL, size + sizeof(PS_DLST_BUCKET), "dlst_newnode")) ) {
		fprintf(stderr,"Not enough memory to allocate list node.\n");
/*		raise(SIGABRT);*/
		return NULL;
		}

	return PS_DLST_USERSPACE(node);		/* Return pointer to user space */
}

PUBLIC void dlst_freenode(DLIST *l, void *node)
/****************************************************************************
*
...
****************************************************************************/
{
	l->free(NULL, PS_DLST_HEADER(node));
}

PUBLIC DLIST *dlst_init(void* (*allocproc)(PSDoc *ps, size_t size, const char *caller), void* (*reallocproc)(PSDoc *ps, void *mem, size_t size, const char *caller), void  (*freeproc)(PSDoc *ps, void *mem))
/****************************************************************************
*
* Function:		dlst_init
...
****************************************************************************/
{
	...
	if(allocproc == NULL ||
	   reallocproc == NULL ||
		...
		if ((l = (DLIST*) (* allocproc) (NULL, sizeof(DLIST), "dlst_init")) != NULL) {
		l->count = 0;
		l->head = &(l->hz[0]);
		l->z = &(l->hz[1]);
	...
	l->malloc = allocproc;
	l->realloc = reallocproc;
	l->free = freeproc;

	return l;
}

PUBLIC void dlst_kill(DLIST *l,void (*freeNode)(DLIST *l, void *node))
/****************************************************************************
*
* Function:		dlst_kill
...
****************************************************************************/
{
	...
	while (n != l->z) {			/* Free all nodes in list				*/
		p = n;
		n = n->next;
		(*freeNode)(l, PS_DLST_USERSPACE(p));
		}
	l->free(NULL, l);					/* Free the list itself					*/
}
...
*				macro PS_DLST_HEAD in place of 'after'. ie:
*
*					dlst_insertafter(mylist,node,PS_DLST_HEAD(mylist));
*
****************************************************************************/
{
	PS_DLST_BUCKET	*n = PS_DLST_HEADER(node),*a = PS_DLST_HEADER(after);
...
****************************************************************************/
{
	PS_DLST_BUCKET	*n = PS_DLST_HEADER(node);

	node = PS_DLST_USERSPACE(n->next);
	n->next->next->prev = n;
	n->next = n->next->next;
	l->count--;
	return node;
...
****************************************************************************/
{
	...
	n = l->head->next;
	return (n == l->z ? NULL : PS_DLST_USERSPACE(n));
}
...
****************************************************************************/
{
	...
	n = l->z->prev;
	return (n == l->head ? NULL : PS_DLST_USERSPACE(n));
}
...
****************************************************************************/
{
	PS_DLST_BUCKET	*n = PS_DLST_HEADER(prev);

	n = n->next;
	return (n == n->next ? NULL : PS_DLST_USERSPACE(n));
}
...
****************************************************************************/
{
	PS_DLST_BUCKET	*n = PS_DLST_HEADER(next);

	n = n->prev;
	return (n == n->prev ? NULL : PS_DLST_USERSPACE(n));
}

/* Static globals required by merge()	*/

static PS_DLST_BUCKET	*z;
static int 			(*cmp)(void*,void*);

PRIVATE PS_DLST_BUCKET *merge(PS_DLST_BUCKET *a,PS_DLST_BUCKET *b,PS_DLST_BUCKET **end)
/****************************************************************************
*
* Function:		merge
...
****************************************************************************/
{
	...
	c = z;
	while (a != z && b != z) {
		if ((*cmp)(PS_DLST_USERSPACE(a),PS_DLST_USERSPACE(b)) <= 0) {
...
{
	int			i,N;
	PS_DLST_BUCKET	*a,*b;		/* Pointers to sublists to merge			*/
	PS_DLST_BUCKET	*c;			/* Pointer to end of sorted sublists		*/
	PS_DLST_BUCKET	*head;		/* Pointer to dummy head node for list		*/
	PS_DLST_BUCKET	*todo;		/* Pointer to sublists yet to be sorted		*/
	PS_DLST_BUCKET	*t;			/* Temporary								*/
